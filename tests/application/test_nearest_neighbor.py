...
from tests.conftest import make_order, make_vehicle, make_warehouse

solver = NearestNeighborSolver()
calculator = HaversineDistanceCalculator()


def _matrix(orders, warehouses):
    locations = [w.location for w in warehouses] + [o.location for o in orders]
    return calculator.compute_matrix(locations)


def _solve(orders, vehicles, warehouses):
    matrix = _matrix(orders, warehouses)
    return solver.solve(orders, vehicles, warehouses, matrix)


# --- basic assignment ---

def test_all_orders_assigned_when_capacity_sufficient(small_orders, small_vehicles, small_warehouses):
    result = _solve(small_orders, small_vehicles, small_warehouses)
...
def test_vehicles_used_does_not_exceed_available(small_orders, small_vehicles, small_warehouses):
    result = _solve(small_orders, small_vehicles, small_warehouses)
    assert result.vehicles_used <= len(small_vehicles)


def test_total_distance_positive(small_orders, small_vehicles, small_warehouses):
    result = _solve(small_orders, small_vehicles, small_warehouses)
    assert result.total_distance > 0


def test_total_distance_equals_sum_of_routes(small_orders, small_vehicles, small_warehouses):
    result = _solve(small_orders, small_vehicles, small_warehouses)
...
def test_no_route_exceeds_weight_capacity(small_orders, small_vehicles, small_warehouses):
    result = _solve(small_orders, small_vehicles, small_warehouses)
...
def test_no_route_exceeds_volume_capacity(small_orders, small_vehicles, small_warehouses):
    result = _solve(small_orders, small_vehicles, small_warehouses)
    vehicle_map = {v.vehicle_id: v for v in small_vehicles}
    order_map = {o.order_id: o for o in small_orders}
    for route in result.routes:
...
def test_order_unassigned_when_too_heavy():
    ...
    result = _solve([heavy_order], [vehicle], [wh])
...
def test_order_unassigned_when_too_voluminous():
    ...
    result = _solve([bulky_order], [vehicle], [wh])
...
def test_no_unassigned_with_generous_capacity():
    ...
    result = _solve(orders, [vehicle], [wh])
    assert result.unassigned_orders == []


# --- edge cases ---

def test_empty_orders_returns_empty_result(small_vehicles, small_warehouses):
    result = _solve([], small_vehicles, small_warehouses)
...
def test_single_order_single_vehicle():
    ...
    result = _solve([order], [vehicle], [wh])
